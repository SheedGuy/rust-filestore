...
use google_cloud_storage::http::objects::{upload::*, Object, download::Range};
use anyhow::Result;
...
impl GCSClient {
    ...
    pub async fn list_buckets(self: &Self, project_id: &str) -> Result<Vec<Bucket>> {
        let resp = self.gcs_client.list_buckets(
            &ListBucketsRequest{
    ...
    pub async fn upload_image(
        ...
        name: &str, 
        image_data: Vec<u8>, 
        content_type: &str) -> Result<Object> {
        ...
        Ok(self.gcs_client.upload_object(&upload_req,image_data,&upload_type).await?)
    }
    ...
    pub async fn get_image(
        ...
        Ok(self.gcs_client.download_object(&download_req, &Range::default()).await?)            
    }
